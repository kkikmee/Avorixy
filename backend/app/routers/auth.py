...
async def register(payload: UserCreate, db: AsyncSession = Depends(get_db)):
    ...
    # Создаём дефолтные настройки dashboard
    settings = DashboardSettings(
        user_id=user.id,
        visible_categories=[],
        visible_days=["monday", "tuesday", "wednesday", "thursday", "friday", "saturday", "sunday"],
        visible_time_slots=["morning", "afternoon", "evening", "night"],
    )
    db.add(settings)
    await db.commit()
