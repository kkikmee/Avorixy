...
async def _get_slot_for_user(slot_id: int, user: User, db: AsyncSession) -> TimeSlot:
    """Проверяем, что слот принадлежит текущему пользователю."""
    result = await db.execute(
        select(TimeSlot)
        .join(DaySchedule)
        .join(Category)
        .where(TimeSlot.id == slot_id, Category.user_id == user.id)
    )
...
async def list_tasks_by_slot(
    ...
    result = await db.execute(
        select(Task)
        .where(Task.time_slot_id == slot_id)
        .order_by(Task.sort_order, Task.scheduled_time)
    )
...
async def update_task(
    ...
    result = await db.execute(
        ...
        .join(DaySchedule)
        .join(Category)
        .where(Task.id == task_id, Category.user_id == current_user.id)
    )
